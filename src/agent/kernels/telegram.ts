...
export const telegramConfig: KernelConfig = {
  ...
  buildPrompt(_ctx: AgentContext): string {
    return [
      ...
      '- Use `inline code` or ```code blocks``` for anything the user might want to copy — phone numbers, addresses, commands, IDs\n' +
      '- No [[wikilinks]] — plain note names only, but links like this: [example](https://example.com)\n' +
